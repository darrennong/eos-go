...
func (s Signature) Verify(hash []byte, pubKey PublicKey) bool {
	if s.Curve != CurveK1 {
		fmt.Println("WARN: github.com/darrennong/pc-go/ecc library does not support the R1 curve yet")
		return false
	}
...
func (s Signature) PublicKey(hash []byte) (out PublicKey, err error) {
	if s.Curve != CurveK1 {
		return out, fmt.Errorf("WARN: github.com/darrennong/pc-go/ecc library does not support the R1 curve yet")
	}
